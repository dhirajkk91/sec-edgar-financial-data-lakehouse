...
Transient connection errors, timeouts, HTTP `429`, and HTTP `500`, `502`, `503`, and `504` receive at most three total attempts per request, with waits of one and two seconds before retries. A valid `Retry-After` on `429` or `503` can extend those waits; if it asks for more than 60 seconds, stop the run instead of retrying early. The same request pacing applies to index requests, document requests, and retries. An access-block response or HTTP `403` stops requests for that run, even for an optional Data File. Other HTTP errors, unsafe names, empty responses, and content-validation failures are not retried. Ordinary optional Data File failures are recorded and later files are still attempted; a required-file failure stops further downloads. Record network attempt counts and final failure reasons without assigning network attempts to storage or metadata artifacts.
